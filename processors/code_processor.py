...
class CodeProcessor:
    ...
    def process(self, code: str) -> Dict[str, Any]:
        ...
        """
        # Initialize result
        result = {"content": code, "content_type": "code", "metadata": {}}
        ...
        if self.llm:
            result["documentation"] = self._generate_documentation(
                code, detected_language
            )

        logger.debug(
            f"Processed code input: {len(code)} chars, {result['metadata']['line_count']} lines, language: {detected_language}"
        )
        return result
    ...
    def _detect_language(self, code: str) -> str:
        ...
        if scores:
            ...
            if max_score > 0:
                ...
                max_langs = [
                    lang for lang, score in scores.items() if score == max_score
                ]
                if len(max_langs) > 1:
    ...
    def _analyze_structure(self, code: str, language: str) -> Dict[str, Any]:
        ...
        if language == "python":
            ...
            for match in re.finditer(r"^(\w+)\s*=\s*(?!\s*lambda)", code, re.MULTILINE):
                ...
                if (
                    var_name not in structure["variables"]
                    and var_name not in structure["functions"]
                ):
                    structure["variables"].append(var_name)
        elif language in ["javascript", "typescript"]:
            # Match variable declarations
            for match in re.finditer(
                r"(const|let|var)\s+(\w+)\s*=", code, re.MULTILINE
            ):
    ...
    def _extract_dependencies(self, code: str, language: str) -> List[str]:
        ...
        if language == "python":
            ...
            dependencies = [
                dep for dep in dependencies if dep.lower() in common_packages
            ]
    ...
    def execute_code(
        ...
        if (
            self.settings
            and hasattr(self.settings, "code_execution")
            and not self.settings.code_execution.get("enabled", False)
        ):
    ...
    def format_code(self, code: str, language: str) -> str:
        ...
        if language == "python":
            try:
                ...
                logger.warning(
                    "black is not installed. Install it with: pip install black"
                )
                return code
            except Exception as e:
        ...
        # JavaScript/TypeScript code formatting with prettier
            ...
            try:
                ...
                # Create a temporary file
                with tempfile.NamedTemporaryFile(
                    suffix=f".{language}", delete=False
                ) as temp_file:
                    temp_file_path = temp_file.name
                    temp_file.write(code.encode("utf-8"))
    ...
    def analyze_complexity(self, code: str, language: str) -> Dict[str, Any]:
        ...
        if self.llm:
            try:
                ...
                cc_match = re.search(
                    r"cyclomatic complexity[:\s]*(\d+)", analysis_text, re.IGNORECASE
                )
                if cc_match:
                    result["cyclomatic_complexity"] = int(cc_match.group(1))

                cog_match = re.search(
                    r"cognitive complexity[:\s]*(\d+)", analysis_text, re.IGNORECASE
                )
                if cog_match:
                    result["cognitive_complexity"] = int(cog_match.group(1))

                mi_match = re.search(
                    r"maintainability[:\s]*(\d+)", analysis_text, re.IGNORECASE
                )
                if mi_match:
