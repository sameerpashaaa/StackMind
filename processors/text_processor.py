...
class TextProcessor:
    ...
    def process(self, text: str) -> Dict[str, Any]:
        ...
        """
        # Initialize result
        result = {"content": text, "content_type": "text", "metadata": {}}
        ...
        logger.debug(
            f"Processed text input: {len(text)} chars, {result['metadata']['word_count']} words"
        )
        return result
    ...
    def _detect_structure(self, text: str) -> str:
        ...
        if re.search(r"^\s*Q\s*:\s*.+$", text, re.MULTILINE) and re.search(
            r"^\s*A\s*:\s*.+$", text, re.MULTILINE
        ):
            return "qa_format"
