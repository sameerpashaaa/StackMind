...
try:
    ...
    logging.warning(
        "Some image processing dependencies are not installed. "
...
class ImageProcessor:
    ...
    def _detect_handwriting(self, gray_image: np.ndarray) -> bool:
        ...
        try:
            ...
            if len(significant_contours) > 20:
                ...
                avg_perimeter = np.mean(
                    [cv2.arcLength(c, True) for c in significant_contours]
                )
            ...
            message = [
                HumanMessage(
                    content=[
                        {
                            "type": "text",
                            "text": "Describe this image in detail, including any text, diagrams, or relevant information visible.",
                        },
                        {
                            "type": "image_url",
                            "image_url": {
    ...
    def analyze_diagram(self, image_path: str) -> Dict[str, Any]:
        ...
        if "error" in result or (
            result["metadata"]["image_type"] != "diagram"
            and result["metadata"]["image_type"] != "chart"
        ):
            return result
                ...
                message = [
                    HumanMessage(
                        content=[
                            {
                                "type": "text",
                                "text": "Analyze this diagram/chart in detail. Identify the type of diagram, key components, relationships, data points, trends, and any conclusions that can be drawn. If there's text, include it in your analysis.",
                            },
                            {
                                "type": "image_url",
                                "image_url": {
    ...
    def extract_math_from_image(self, image_path: str) -> Dict[str, Any]:
        ...
        if self.llm:
            try:
                ...
                message = [
                    HumanMessage(
                        content=[
                            {
                                "type": "text",
                                "text": "Extract and transcribe all mathematical expressions, equations, formulas, or calculations visible in this image. Format them using LaTeX notation where appropriate. If there are multiple expressions, number them and provide them in the order they appear.",
                            },
                            {
                                "type": "image_url",
                                "image_url": {
                ...
                if len(math_extraction) > 10 and (
                    "=" in math_extraction or "\\" in math_extraction
                ):
                    ...
                    explanation_message = [
                        HumanMessage(
                            content=[
                                {
                                    "type": "text",
                                    "text": f"Explain the following mathematical expressions in detail, including what they represent and how they might be used:\n\n{math_extraction}",
                                },
                            ]
                        )
                    ]
    ...
    def extract_code_from_image(self, image_path: str) -> Dict[str, Any]:
        ...
        if self.llm:
            try:
                ...
                message = [
                    HumanMessage(
                        content=[
                            {
                                "type": "text",
                                "text": "Extract and transcribe all code visible in this image. Preserve indentation and formatting as much as possible. Identify the programming language if possible.",
                            },
                            {
                                "type": "image_url",
                                "image_url": {
                ...
                if len(code_extraction) > 20 and (
                    "(" in code_extraction
                    or "{" in code_extraction
                    or "=" in code_extraction
                ):
                    # Create message for explanation
                    explanation_message = [
                        HumanMessage(
                            content=[
                                {
                                    "type": "text",
                                    "text": f"Explain the following code in detail, including what it does, how it works, and any potential issues or improvements:\n\n{code_extraction}",
                                },
                            ]
                        )
                    ]
