...
class ProblemSolverCLI:
    ...
    def _print_header(self):
        ...
        print(
            f"{Fore.CYAN}║ {Fore.WHITE}{Style.BRIGHT}AI Problem Solver{Style.RESET_ALL}{Fore.CYAN} - Multi-Domain Problem Solving System ║"
        )
        print(
            f"{Fore.CYAN}╚═══════════════════════════════════════════════════════════════╝"
        )
        print(
            f"{Fore.YELLOW}Type '{Fore.WHITE}help{Fore.YELLOW}' for available commands or '{Fore.WHITE}exit{Fore.YELLOW}' to quit."
        )
        print(
            f"{Fore.YELLOW}Session ID: {Fore.WHITE}{self.session_id}{Style.RESET_ALL}\n"
        )
    ...
    def _print_help(self):
        ...
        print(
            f"  {Fore.GREEN}exit{Style.RESET_ALL}, {Fore.GREEN}quit{Style.RESET_ALL}             - Exit the application"
        )
        print(
            f"  {Fore.GREEN}clear{Style.RESET_ALL}                  - Clear the screen"
        )
        print(
            f"  {Fore.GREEN}history{Style.RESET_ALL}                - Show solution history"
        )
        print(
            f"  {Fore.GREEN}show <solution_num>{Style.RESET_ALL}    - Show details of a specific solution"
        )
        print(
            f"  {Fore.GREEN}explain{Style.RESET_ALL}                - Explain the current solution in more detail"
        )
        print(
            f"  {Fore.GREEN}verify{Style.RESET_ALL}                 - Verify the current solution"
        )
        print(
            f"  {Fore.GREEN}alternatives{Style.RESET_ALL}           - Generate alternative solutions"
        )
        print(
            f"  {Fore.GREEN}feedback <text>{Style.RESET_ALL}        - Provide feedback on the current solution"
        )
        print(
            f"  {Fore.GREEN}refine <text>{Style.RESET_ALL}          - Refine the current solution based on feedback"
        )
        print(
            f"  {Fore.GREEN}domain <domain_name>{Style.RESET_ALL}   - Specify a domain for the next problem (math, code, science, general)"
        )
        print(
            f"  {Fore.GREEN}image <file_path>{Style.RESET_ALL}      - Process an image file"
        )
        print(
            f"  {Fore.GREEN}voice <file_path>{Style.RESET_ALL}      - Process a voice recording file"
        )
        print(
            f"  {Fore.GREEN}code <file_path>{Style.RESET_ALL}       - Process code from a file"
        )
        print(
            f"  {Fore.GREEN}save <file_path>{Style.RESET_ALL}       - Save the current solution to a file"
        )
        print(
            f"  {Fore.GREEN}export <file_path>{Style.RESET_ALL}     - Export the entire session to a JSON file"
        )
        print(
    ...
    def _print_solution(self, solution: Dict[str, Any], detailed: bool = False):
        ...
        print(
            f"{Fore.YELLOW}Confidence:{Style.RESET_ALL} {solution.get('confidence', 0.0):.2f}\n"
        )
    ...
    def _show_history(self):
        ...
        for i, solution in enumerate(self.solutions, 1):
            timestamp = solution.get("timestamp", datetime.now()).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            print(
                f"{Fore.CYAN}{i}.{Style.RESET_ALL} [{timestamp}] {Fore.YELLOW}{solution['input_type']}:{Style.RESET_ALL} {solution['input_preview']} {Fore.GREEN}({solution['result']['domain']}){Style.RESET_ALL}"
            )
        print()
    ...
    def _show_solution_details(self, solution_num: int):
        ...
        if not self.solutions or solution_num < 1 or solution_num > len(self.solutions):
            print(
                f"{Fore.RED}Invalid solution number. Use 'history' to see available solutions.{Style.RESET_ALL}"
            )
            return
    ...
    def _process_text_input(self, text: str, domain_hint: Optional[str] = None):
        ...
        try:
            print(
                f"\n{Fore.YELLOW}Thinking... (this may take a minute){Style.RESET_ALL}",
                flush=True,
            )

            # Process the input through the agent's solve pipeline
            result = self.agent.solve_problem(input_data=text, input_type="text")
    ...
    def _process_image_input(self, file_path: str, domain_hint: Optional[str] = None):
        ...
        try:
            ...
            # Process the image
            result = self.agent.solve_problem(input_data=file_path, input_type="image")
    ...
    def _process_voice_input(self, file_path: str, domain_hint: Optional[str] = None):
        ...
        try:
            ...
            result = self.agent.solve_problem(input_data=file_path, input_type="voice")
            ...
            print(
                f"{Fore.RED}Error processing voice recording: {str(e)}{Style.RESET_ALL}"
            )
    ...
    def _process_code_input(self, file_path: str):
        ...
        try:
            ...
            # Process the code
            result = self.agent.solve_problem(input_data=code, input_type="code")
    ...
    def _explain_solution(self):
        ...
        if not self.current_solution_id:
            print(
                f"{Fore.YELLOW}No current solution to explain. Solve a problem first.{Style.RESET_ALL}"
            )
            return
        ...
        try:
            ...
            explanation = self.agent.explain_solution(
                session_id=self.session_id, solution_id=self.current_solution_id
            )
    ...
    def _verify_solution(self):
        ...
        if not self.current_solution_id:
            print(
                f"{Fore.YELLOW}No current solution to verify. Solve a problem first.{Style.RESET_ALL}"
            )
            return
        ...
        try:
            ...
            print(
                f"{Fore.YELLOW}Is Correct:{Style.RESET_ALL} {verification_result['is_correct']}"
            )
            print(
                f"{Fore.YELLOW}Confidence:{Style.RESET_ALL} {verification_result['confidence']:.2f}\n"
            )
            print(
                f"{Fore.GREEN}Verification Details:{Style.RESET_ALL}\n{verification_result['details']}\n"
            )
    ...
    def _generate_alternatives(self):
        ...
        if not self.current_solution_id:
            print(
                f"{Fore.YELLOW}No current solution to generate alternatives for. Solve a problem first.{Style.RESET_ALL}"
            )
            return
        ...
        try:
            ...
            for i, alt in enumerate(alternatives, 1):
                ...
                print(
                    f"{Fore.YELLOW}Confidence:{Style.RESET_ALL} {alt.get('confidence', 0.0):.2f}\n"
                )
                print(f"{Fore.GREEN}Solution:{Style.RESET_ALL}\n{alt['solution']}\n")
    ...
    def _provide_feedback(self, feedback_text: str):
        ...
        if not self.current_solution_id:
            print(
                f"{Fore.YELLOW}No current solution to provide feedback for. Solve a problem first.{Style.RESET_ALL}"
            )
            return
        ...
        try:
            ...
            current_solution["feedback"].append(
                {"timestamp": datetime.now(), "text": feedback_text}
            )

            print(
                f"{Fore.GREEN}Feedback recorded. Use 'refine' to improve the solution based on your feedback.{Style.RESET_ALL}"
            )
    ...
    def _refine_solution(self, refinement_text: str):
        ...
        if not self.current_solution_id:
            print(
                f"{Fore.YELLOW}No current solution to refine. Solve a problem first.{Style.RESET_ALL}"
            )
            return
                ...
                try:
                    ...
                    if not user_input:
                        ...
                        if domain in ["math", "code", "science", "general"]:
                            domain_hint = domain
                            print(
                                f"{Fore.GREEN}Domain set to {domain_hint}.{Style.RESET_ALL}"
                            )
                        else:
                            print(
                                f"{Fore.RED}Invalid domain. Use 'math', 'code', 'science', or 'general'.{Style.RESET_ALL}"
                            )
                        ...
                        if not self.current_solution_id:
                            print(
                                f"{Fore.YELLOW}No current solution to save. Solve a problem first.{Style.RESET_ALL}"
                            )
                            continue
                        ...
                        current_solution = next(
                            (
                                ...
                                if s["id"] == self.current_solution_id
                            ),
                            None,
                        )
                        if current_solution:
...
def parse_arguments():
    ...
    parser.add_argument(
        "--domain",
        type=str,
        choices=["math", "code", "science", "general"],
        help="Problem domain",
    )
    parser.add_argument("--image", type=str, help="Path to image file to process")
    parser.add_argument(
        "--voice", type=str, help="Path to voice recording file to process"
    )
...
def main():
    ...
    if args.problem or args.image or args.voice or args.code:
        ...
        if args.output and cli.current_solution_id:
            current_solution = next(
                (s for s in cli.solutions if s["id"] == cli.current_solution_id), None
            )
            if current_solution:
