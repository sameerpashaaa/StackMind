...
class GeneralSolver:
    ...
    def solve(
        ...
        messages = [
            SystemMessage(
                content="You are an AI assistant that solves problems using a systematic approach."
            ),
            HumanMessage(
                content=template.format(
                    problem=problem_content, plan=plan_text, feedback=feedback_text
                )
            ),
        ]
    ...
    def _create_solution_prompt(
        self, has_plan: bool = False, has_feedback: bool = False
    ) -> PromptTemplate:
        """
        ...
        return PromptTemplate(
            template=template, input_variables=["problem", "plan", "feedback"]
        )
    ...
    def verify(self, solution: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not problem_id:
            ...
            return {"is_correct": False, "error": "Missing problem_id in solution"}
        ...
        prompt = PromptTemplate(template=template, input_variables=["solution"])

        # Generate verification using the LLM
        messages = [
            SystemMessage(
                content="You are an AI assistant that verifies solutions for correctness and completeness."
            ),
        ...
        is_correct = True
        if (
            "incorrect" in verification_text.lower()
            or "error" in verification_text.lower()
            or "wrong" in verification_text.lower()
            or "mistake" in verification_text.lower()
            or "issue" in verification_text.lower()
            or "problem" in verification_text.lower()
        ):
            is_correct = False
    ...
    def explain(self, solution: Dict[str, Any]) -> Dict[str, Any]:
        ...
        messages = [
            SystemMessage(
                content="You are an AI assistant that provides detailed explanations of solutions."
            ),
            HumanMessage(content=prompt.format(solution=solution_content)),
        ]
