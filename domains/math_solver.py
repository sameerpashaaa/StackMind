...
import pandas as pd
import sympy
from sympy import (Matrix, diff, expand, factor, integrate, simplify, solve,
                   symbols)
...
class MathSolver:
    ...
    def verify_solution(self, problem: str, solution: Any) -> Dict[str, Any]:
        ...
        """
        result = {"verified": False, "explanation": "", "alternative_solution": None}
        ...
        try:
            ...
            if problem_type == "arithmetic":
                ...
                solution_data = self._solve_arithmetic(problem, expressions)
                result["verified"] = self._compare_solutions(
                    solution, solution_data["solution"]
                )
            elif problem_type == "algebraic_equation":
                # For equations, substitute the solution back into the equation
                result["verified"] = self._verify_equation_solution(
                    expressions, solution
                )
            elif problem_type == "algebraic_expression":
                # For expressions, compare simplified forms
                solution_data = self._solve_algebraic_expression(problem, expressions)
                result["verified"] = self._compare_expressions(
                    solution, solution_data["solution"]
                )
            else:
                ...
                if self.llm:
                    ...
                    result["explanation"] = (
                        "Verification not available for this problem type without LLM."
                    )
    ...
    def _extract_expressions(self, problem: str) -> List[str]:
        ...
        equation_expressions = re.findall(
            r"([\w\d\s+\-*/^(){}\[\]]+=[\w\d\s+\-*/^(){}\[\]]+)", problem
        )
        expressions.extend(equation_expressions)

        # Extract expressions that look like mathematical formulas
        formula_expressions = re.findall(
            r"([\d]+[+\-*/^][\d]+|[a-zA-Z]\([a-zA-Z]\)|\\frac\{.*\}\{.*\}|\\sqrt\{.*\})",
            problem,
        )
    ...
    def _solve_arithmetic(self, problem: str, expressions: List[str]) -> Dict[str, Any]:
        ...
        """
        result = {"solution": None, "steps": [], "explanation": ""}
        ...
        try:
            ...
            while "(" in current_expr:
                ...
                if inner_expr:
                    ...
                    current_expr = current_expr.replace(
                        inner_expr.group(0), str(inner_result), 1
                    )
            ...
            result["steps"] = steps
            result["explanation"] = (
                "Arithmetic calculation performed step by step, following order of operations."
            )
    ...
    def _solve_algebraic_equation(
        self, problem: str, expressions: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if "=" in equation:
                ...
                steps.append(
                    f"Move all terms to the left side: {left_side} - ({right_side}) = 0"
                )
                ...
                if solutions:
                    steps.append(
                        f"Solutions: {symbol_to_solve} = {', '.join(str(sol) for sol in solutions)}"
                    )
                    result["solution"] = {
                        str(symbol_to_solve): [str(sol) for sol in solutions]
                    }
                else:
                ...
                result["steps"] = steps
                result["explanation"] = (
                    f"Solved the equation for {symbol_to_solve} by isolating the variable."
                )
    ...
    def _solve_algebraic_expression(
        self, problem: str, expressions: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            result["steps"] = steps
            result["explanation"] = (
                f"Performed {operation} on the algebraic expression."
            )
    ...
    def _solve_differentiation(
        ...
        try:
            ...
            result["steps"] = steps
            result["explanation"] = (
                f"Differentiated the expression with respect to {var_name} using calculus rules."
            )
    ...
    def _solve_integration(
        ...
        try:
            ...
            # Default to x if not specified
            var_match = (
                re.search(r"\\int_\{[^}]*\}\^\{[^}]*\}.*?d([a-zA-Z])", problem)
                or re.search(r"\\int.*?d([a-zA-Z])", problem)
                or re.search(r"∫.*?d([a-zA-Z])", problem)
                or re.search(r"integrate.*with respect to\s+([a-zA-Z])", problem)
            )
            ...
            limits_match = (
                re.search(r"from\s+([\d\.-]+)\s+to\s+([\d\.-]+)", problem)
                or re.search(r"\\int_\{([\d\.-]+)\}\^\{([\d\.-]+)\}", problem)
                or re.search(r"∫_\{([\d\.-]+)\}\^\{([\d\.-]+)\}", problem)
            )
            ...
            if limits_match:
                ...
                steps.append(
                    f"Integrate with respect to {var_name} from {lower_limit} to {upper_limit}"
                )
            ...
            result["steps"] = steps
            result["explanation"] = (
                f"Integrated the expression with respect to {var_name} using calculus rules."
            )
    ...
    def _solve_linear_algebra(
        ...
        try:
            ...
            if "determinant" in problem_lower:
                ...
                "system of equation" in problem_lower
                or "linear system" in problem_lower
            ):
    ...
    def _solve_eigenvalue(self, problem: str, expressions: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            for val, mult in eigenvals.items():
                eigenvalue_dict[str(val)] = {"multiplicity": mult, "eigenvectors": []}
            ...
            result["steps"] = steps
            result["explanation"] = (
                "Calculated the eigenvalues and eigenvectors of the matrix."
            )
    ...
    def _solve_matrix_inverse(
        ...
        try:
            ...
            result["steps"] = steps
            result["explanation"] = (
                "Calculated the inverse of the matrix using the adjugate method."
            )
    ...
    def _solve_linear_system(
        ...
        try:
            ...
            result["steps"] = steps
            result["explanation"] = (
                "Solved the system of linear equations using elimination and substitution methods."
            )
    ...
    def _solve_dot_product(
        ...
        try:
            ...
            steps.append(
                f"Calculate dot product: {' + '.join(f'{a} * {b}' for a, b in zip(vector1, vector2))}"
            )
            ...
            result["steps"] = steps
            result["explanation"] = (
                "Calculated the dot product of the two vectors by multiplying corresponding components and summing the results."
            )
    ...
    def _solve_cross_product(
        ...
        try:
            ...
            steps.append(
                f"i: {vector1[1]} * {vector2[2]} - {vector1[2]} * {vector2[1]} = {cross_product[0]}"
            )
            steps.append(
                f"j: {vector1[2]} * {vector2[0]} - {vector1[0]} * {vector2[2]} = {cross_product[1]}"
            )
            steps.append(
                f"k: {vector1[0]} * {vector2[1]} - {vector1[1]} * {vector2[0]} = {cross_product[2]}"
            )
            steps.append(
                f"Result: [{cross_product[0]}, {cross_product[1]}, {cross_product[2]}]"
            )

            result["solution"] = cross_product
            result["steps"] = steps
            result["explanation"] = (
                "Calculated the cross product of the two 3D vectors using the determinant formula."
            )
    ...
    def _solve_statistics(self, problem: str, expressions: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            if "mean" in problem_lower or "average" in problem_lower:
                ...
                steps.append(
                    f"Calculate standard deviation: sqrt({variance}) = {std_dev}"
                )
                result["solution"] = std_dev
                result["explanation"] = (
                    "Calculated the standard deviation of the data set."
                )
                ...
                result["explanation"] = (
                    "Calculated basic statistical measures for the data set."
                )
        ...
        try:
            ...
            if "area" in problem_lower and "triangle" in problem_lower:
                ...
                if len(values) >= 2:
                    ...
                    result["explanation"] = (
                        "Calculated the area of a triangle using the formula: Area = 0.5 * base * height."
                    )
                else:
                    result["error"] = (
                        "Insufficient values for triangle area calculation."
                    )
                ...
                if len(values) >= 1:
                    ...
                    result["solution"] = area
                    result["explanation"] = (
                        "Calculated the area of a circle using the formula: Area = π * radius²."
                    )
                else:
                    result["error"] = "Insufficient values for circle area calculation."

            elif "area" in problem_lower and (
                "rectangle" in problem_lower or "square" in problem_lower
            ):
                ...
                if "square" in problem_lower and len(values) >= 1:
                    ...
                    result["solution"] = area
                    result["explanation"] = (
                        "Calculated the area of a square using the formula: Area = side²."
                    )
                elif len(values) >= 2:
                    length = values[0]
                    width = values[1]
                    ...
                    steps.append(
                        f"Rectangle with length = {length} and width = {width}"
                    )
                    steps.append(f"Calculate area: {length} * {width} = {area}")
                    result["solution"] = area
                    result["explanation"] = (
                        "Calculated the area of a rectangle using the formula: Area = length * width."
                    )
                else:
                    result["error"] = (
                        "Insufficient values for rectangle/square area calculation."
                    )
                ...
                if len(values) >= 3:
                    ...
                    result["explanation"] = (
                        "Calculated the perimeter of a triangle by summing all sides."
                    )
                else:
                    result["error"] = (
                        "Insufficient values for triangle perimeter calculation."
                    )
                ...
                if len(values) >= 1:
                    ...
                    steps.append(f"Circle with radius = {radius}")
                    steps.append(
                        f"Calculate circumference: 2 * π * {radius} = {circumference}"
                    )
                    result["solution"] = circumference
                    result["explanation"] = (
                        "Calculated the circumference of a circle using the formula: Circumference = 2 * π * radius."
                    )
                else:
                    result["error"] = (
                        "Insufficient values for circle circumference calculation."
                    )

            elif "perimeter" in problem_lower and (
                "rectangle" in problem_lower or "square" in problem_lower
            ):
                ...
                if "square" in problem_lower and len(values) >= 1:
                    ...
                    result["solution"] = perimeter
                    result["explanation"] = (
                        "Calculated the perimeter of a square using the formula: Perimeter = 4 * side."
                    )
                elif len(values) >= 2:
                    length = values[0]
                    width = values[1]
                    ...
                    steps.append(
                        f"Calculate perimeter: 2 * ({length} + {width}) = {perimeter}"
                    )
                    result["solution"] = perimeter
                    result["explanation"] = (
                        "Calculated the perimeter of a rectangle using the formula: Perimeter = 2 * (length + width)."
                    )
                else:
                    result["error"] = (
                        "Insufficient values for rectangle/square perimeter calculation."
                    )
                ...
                if len(values) >= 1:
                    ...
                    result["explanation"] = (
                        "Calculated the volume of a sphere using the formula: Volume = (4/3) * π * radius³."
                    )
                else:
                    result["error"] = (
                        "Insufficient values for sphere volume calculation."
                    )
                ...
                if len(values) >= 1:
                    ...
                    result["solution"] = volume
                    result["explanation"] = (
                        "Calculated the volume of a cube using the formula: Volume = side³."
                    )
                else:
                ...
                if len(values) >= 2:
                    ...
                    steps.append(
                        f"Cylinder with radius = {radius} and height = {height}"
                    )
                    steps.append(
                        f"Calculate volume: π * {radius}² * {height} = {volume}"
                    )
                    result["solution"] = volume
                    result["explanation"] = (
                        "Calculated the volume of a cylinder using the formula: Volume = π * radius² * height."
                    )
                else:
                    result["error"] = (
                        "Insufficient values for cylinder volume calculation."
                    )

            elif "pythagorean" in problem_lower or (
                "triangle" in problem_lower and "right" in problem_lower
            ):
                # Pythagorean theorem
                if len(values) >= 2:
                    a = values[0]
                    ...
                    steps.append(
                        f"Calculate hypotenuse using Pythagorean theorem: c = √(a² + b²) = √({a}² + {b}²) = {c}"
                    )
                    result["solution"] = c
                    result["explanation"] = (
                        "Calculated the hypotenuse of a right triangle using the Pythagorean theorem: c = √(a² + b²)."
                    )
                else:
                    result["error"] = (
                        "Insufficient values for Pythagorean theorem calculation."
                    )
    ...
    def _solve_with_llm(
        ...
        if not self.llm:
            result["error"] = (
                "Language model not available for solving this problem type."
            )
            return result
        ...
        try:
            ...
            messages = [
                SystemMessage(
                    content="You are an expert mathematical problem solver. "
    ...
    def _verify_equation_solution(self, expressions: List[str], solution: Any) -> bool:
        ...
        try:
            ...
            if "=" in equation:
                ...
                # Find all symbols (variables)
                symbols_in_expr = list(
                    left_expr.free_symbols.union(right_expr.free_symbols)
                )
    ...
    def _can_visualize(self, problem_type: str, solution: Any) -> bool:
        """Check if the solution can be visualized."""
        return False

    def _generate_visualization(self, problem_type: str, solution: Any) -> Optional[str]:
        """Generate visualization for the solution."""
        return None

    def _generate_explanation(self, problem: str, solution: Any, steps: List[str] = None) -> str:
        """Generate explanation for the math problem solution."""
        if self.llm:
            try:
                from langchain.schema import AIMessage, HumanMessage, SystemMessage
                messages = [
                    SystemMessage(content="You are an AI assistant that explains mathematical solutions step-by-step."),
    ...
    def _verify_with_llm(self, problem: str, solution: Any) -> Dict[str, Any]:
        """Verify the solution using the LLM."""
        if self.llm:
            try:
                from langchain.schema import AIMessage, HumanMessage, SystemMessage
                messages = [
                    SystemMessage(content="You are an AI assistant that verifies mathematical solutions."),
                    HumanMessage(content=f"Problem: {problem}\nSolution to verify: {solution}\nVerify if this is correct. Return JSON with 'verified' (boolean) and 'explanation' (string).")
                ]
                response = self.llm.generate([messages])
                ...
                try:
                    ...
                    verified = "correct" in text.lower() or "true" in text.lower()
                    return {"verified": verified, "explanation": text}
            except Exception as e:
                logger.error(f"LLM verification failed: {e}")
        return {"verified": False, "explanation": "LLM verification not available."}
