...
class CodeSolver:
    ...
    def __init__(self, llm: Optional[Any] = None):
        ...
        self.supported_languages = {
            ...
            "ruby": {"extension": ".rb", "run_cmd": ["ruby", "{file}"], "comment": "#"},
            "php": {"extension": ".php", "run_cmd": ["php", "{file}"], "comment": "//"},
            "rust": {
                "extension": ".rs",
            ...
            "css": {"extension": ".css", "run_cmd": [], "comment": "/*"},
        }
    ...
    def solve(
        ...
        try:
            ...
            if problem_type == "debug":
                ...
                solution = self._translate_code(
                    problem, code, language, target_language, context
                )
            ...
            if (
                code
                and context
                and context.get("execute_code", False)
    ...
    def _detect_problem_type(self, problem: str) -> str:
        ...
        if any(
            ...
            for keyword in ["debug", "fix", "error", "issue", "not working"]
        ):
            return "debug"
        elif any(
            keyword in problem_lower
            for keyword in ["optimize", "improve", "performance", "faster", "efficient"]
        ):
            return "optimize"
        elif any(
            keyword in problem_lower
            for keyword in ["explain", "understand", "what does", "how does"]
        ):
            return "explain"
        elif any(
            keyword in problem_lower
            for keyword in ["generate", "create", "write", "implement"]
        ):
            return "generate"
        elif any(
            keyword in problem_lower
            for keyword in ["translate", "convert", "port", "from", "to language"]
        ):
            return "translate"
        elif any(
            keyword in problem_lower
            for keyword in ["analyze", "review", "quality", "complexity"]
        ):
            return "analyze"
        else:
            return "general"
    ...
    def _detect_language(self, code: str) -> str:
        ...
        if re.search(r"\bdef\b.*:|\bclass\b.*:|\bimport\b|\bfrom\b.*\bimport\b", code):
            ...
            if "typescript" in code_lower or re.search(
                r":\s*(string|number|boolean|any)\b", code
            ):
                return "typescript"
            else:
                return "javascript"
        elif re.search(
            r"\bpublic\b.*\bclass\b|\bprivate\b|\bprotected\b|\bimport\s+java\.", code
        ):
            return "java"
        elif re.search(r"#include\s*<.*\.h>|\bint\s+main\s*\(", code) and not re.search(
            r"\bclass\b", code
        ):
            return "c"
            ...
            r"\bdef\b|\bend\b|\bmodule\b|\brequire\b", code
    ...
    def _debug_code(
        ...
        """
        result = {"solution": None, "explanation": None, "steps": [], "code": None}
        ...
        try:
            ...
            if error_message:
                messages.append(
                    HumanMessage(content=f"Error message:\n{error_message}")
                )
    ...
    def _optimize_code(
        ...
        try:
            ...
            problem_lower = problem.lower()
            if (
                "performance" in problem_lower
                or "faster" in problem_lower
                or "efficient" in problem_lower
            ):
                optimization_focus = "performance"
            elif (
                "readability" in problem_lower
                or "clean" in problem_lower
                or "maintainable" in problem_lower
            ):
            ...
            messages = [
                SystemMessage(
                    content=f"You are an expert code optimizer. "
    ...
    def _explain_code(
        ...
        """
        result = {"solution": None, "explanation": None, "steps": [], "code": code}
        ...
        try:
            ...
            messages = [
                SystemMessage(
                    content="You are an expert code explainer. "
    ...
    def _translate_code(
        ...
        try:
            ...
            messages = [
                SystemMessage(
                    content=f"You are an expert programmer fluent in both {source_language} and {target_language}. "
    ...
    def _analyze_code(
        ...
        try:
            ...
            messages = [
                SystemMessage(
                    content=f"You are an expert code reviewer and analyzer. "
            ...
            for line in analysis_text.split("\n"):
                if re.search(r"(?i)issues?[:\s]+", line):
                    ...
                    line.strip()[0] == "#" or line.strip().endswith(":")
                ):
                    in_issues = False
                    in_suggestions = False
    ...
    def _solve_with_llm(
        ...
        try:
            ...
            system_content += (
                "Solve the following code-related problem with detailed explanations."
            )
        ...
        try:
            ...
            if "&&" in cmd:
                ...
                process = subprocess.run(
                    first_cmd, capture_output=True, text=True, timeout=10
                )
                ...
                # Run second command
                process = subprocess.run(
                    second_cmd, capture_output=True, text=True, timeout=10
                )
            else:
                process = subprocess.run(
        ...
        try:
            ...
            if language in self.supported_languages:
                ...
                if expected_output and execution_result.get("output"):
                    ...
                    normalized_actual = re.sub(
                        r"\s+", "", execution_result["output"].lower()
                    )
                    result["matches_expected_output"] = (
            ...
            if language in self.supported_languages:
                ...
                if execution_result.get("error"):
                    ...
                    execution_str += (
                        f"Output: {execution_result.get('output', 'No output')}\n"
                    )
            ...
            if expected_output:
                messages.append(
                    HumanMessage(content=f"Expected Output: {expected_output}")
                )
            ...
            result["is_correct"] = (
                "yes" in verification_text[:50]
                or "correct" in verification_text[:50]
                or "solves the problem" in verification_text[:50]
            )

            # If execution passes and either matches expected output or LLM says it's correct
            if result["passes_execution"] and (
                result["matches_expected_output"] or result["is_correct"]
            ):
    ...
    def explain_solution(
        ...
        Returns:
            Dict containing the explanation
        """
        result = {"explanation": None, "summary": None, "sections": [], "error": None}
        ...
        try:
            ...
            detail_instructions = {
                ...
                detail_level.lower(),
                "Explain the main components and logic of the code.",
            )

            # Create prompt for explanation
            messages = [
                SystemMessage(
                    content=f"You are an expert code explainer. "
