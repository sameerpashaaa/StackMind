...
class MemorySystem:
    ...
    def _get_embeddings(self):
        ...
        if os.getenv("MISTRAL_API_KEY"):
            try:
                ...
                embeddings = MistralAIEmbeddings(
                    model="mistral-embed", mistral_api_key=os.getenv("MISTRAL_API_KEY")
                )
        ...
        logger.warning(
            "No embedding provider available — falling back to OpenAI (may fail)"
        )
    ...
    def add(self, item: Dict[str, Any], item_type: str) -> str:
        ...
        if len(self.session_memory[session_key]) > max_history:
            self.session_memory[session_key] = self.session_memory[session_key][
                -max_history:
            ]
        ...
        if self.long_term_memory is not None:
            try:
                ...
                results = self.long_term_memory.similarity_search(
                    query=item_id, k=1, filter={"id": item_id}
                )
    ...
    def search(
        self, query: str, k: int = 5, item_type: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        results = []
            ...
            try:
                ...
                for doc in docs:
                    try:
                        ...
                        logger.warning(
                            f"Failed to parse memory item: {doc.page_content[:100]}..."
                        )
            except Exception as e:
    ...
    def get_recent(
        self, item_type: Optional[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
        if item_type:
            if item_type not in self.MEMORY_TYPES:
    ...
    def get_user_preferences(self) -> Dict[str, Any]:
        ...
        if self.long_term_memory is not None:
            try:
                docs = self.long_term_memory.similarity_search(
                    query="user preferences", k=50, filter={"type": "preference"}
                )
                ...
                logger.error(
                    f"Failed to retrieve preferences from long-term memory: {e}"
                )
