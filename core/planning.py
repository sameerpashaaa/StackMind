...
class PlanningSystem:
    ...
    def create_plan(self, problem: Dict[str, Any], domain: str) -> Dict[str, Any]:
        ...
        prompt = PromptTemplate(
            template=template, input_variables=["problem", "domain"]
        )

        # Generate plan using the LLM
        messages = [
            SystemMessage(
                content="You are an AI assistant that creates detailed solution plans for complex problems."
            ),
            HumanMessage(content=prompt.format(problem=problem_content, domain=domain)),
        ]
        ...
        logger.debug(
            f"Created plan with {len(plan.get('steps', []))} steps for problem: {problem.get('id')}"
        )
        return plan
    ...
    def _parse_plan(self, plan_text: str) -> Dict[str, Any]:
        ...
        plan = {"steps": [], "branches": [], "validations": [], "raw_text": plan_text}
        ...
        try:
            ...
            for line in lines:
                ...
                if (
                    line.startswith(f"Step {step_number}:")
                    or line.startswith(f"Step {step_number}.")
                    or line.startswith(f"{step_number}.")
                    or line.startswith(f"{step_number}:")
                    or line.lower().startswith(f"step {step_number}:")
                    or line.lower().startswith(f"step {step_number}.")
                ):
                    ...
                    if "dependencies:" in lower_line or "depends on:" in lower_line:
                        ...
                        "resources:" in lower_line
                        or "tools:" in lower_line
                        or "requires:" in lower_line
                    ):
                        # Extract resources
            ...
            if (
                "validation" in plan_text.lower()
                or "verify" in plan_text.lower()
                or "check" in plan_text.lower()
            ):
    ...
    def refine_plan(
        self, plan: Dict[str, Any], feedback: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        prompt = PromptTemplate(template=template, input_variables=["plan", "feedback"])

        # Generate refined plan using the LLM
        messages = [
            SystemMessage(
                content="You are an AI assistant that refines solution plans based on user feedback."
            ),
            HumanMessage(
                content=prompt.format(plan=plan_text, feedback=feedback_content)
            ),
        ]
        ...
        logger.debug(
            f"Refined plan with {len(refined_plan.get('steps', []))} steps based on feedback"
        )
        return refined_plan

    def create_alternative_plan(
        self, problem: Dict[str, Any], domain: str, original_plan: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        prompt = PromptTemplate(
            template=template, input_variables=["problem", "domain", "original_plan"]
        )

        # Generate alternative plan using the LLM
        messages = [
            SystemMessage(
                content="You are an AI assistant that creates alternative solution plans for complex problems."
            ),
            HumanMessage(
        ...
        logger.debug(
            f"Created alternative plan with {len(alternative_plan.get('steps', []))} steps"
        )
        return alternative_plan

    def get_plan_visualization(
        self, plan: Dict[str, Any], format: str = "tree"
    ) -> Dict[str, Any]:
        """
        ...
        if format == "tree":
            ...
            tree_data = {"name": "Solution Plan", "children": []}
            ...
            for step in steps:
                ...
                for resource in step.get("resources", []):
                    step_node["children"].append({"name": f"Resource: {resource}"})

                tree_data["children"].append(step_node)

            # Add branches and validations
            if branches:
                branches_node = {"name": "Branches", "children": []}
            ...
            if validations:
                validations_node = {"name": "Validations", "children": []}
                ...
                for dep in step.get("dependencies", []):
                    if dep.startswith("step_"):
                        graph_data["edges"].append(
                            {"from": dep, "to": step.get("id"), "arrows": "to"}
                        )
    ...
    def get_plan_progress(
        self, plan: Dict[str, Any], completed_steps: List[str]
    ) -> Dict[str, Any]:
        """
