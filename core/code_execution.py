...
class CodeExecution:
    def __init__(
        ...
        self.allowed_modules = (
            ...
            else self.default_allowed_modules
        )
        self.blocked_modules = (
            blocked_modules
            if blocked_modules is not None
            else self.default_blocked_modules
        )
    ...
    def _check_node_available(self) -> bool:
        try:
            ...
            logger.warning(
                "Node.js not found. JavaScript execution will not be available."
            )
            return False
        ...
        if language.lower() == "python":
            ...
            for pattern in dangerous_builtins:
                if re.search(pattern, code):
                    return (
                        False,
                        f"Potentially dangerous function: {pattern.strip('\\b')}",
                    )
    ...
    def execute_code(
        self, code: str, language: str, globals_dict: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
