...
class ProblemSolverAgent:
    def __init__(self, settings):
        ...
        logger.info(
            f"Initializing Problem Solver Agent with session ID: {self.session_id}"
        )
    ...
    def _initialize_llm(self) -> BaseLLM:
        ...
        if provider == "openai":
            ...
            logger.warning(
                f"Unsupported LLM provider: {provider}. Falling back to OpenAI."
            )
    ...
    def _initialize_processors(self) -> Dict[str, Any]:
        ...
        if input_settings.get("enable_image", True):
            processors["image"] = ImageProcessor(llm=self.llm, settings=self.settings)

        if input_settings.get("enable_voice", True):
            processors["voice"] = VoiceProcessor()

        if input_settings.get("enable_code", True):
            processors["code"] = CodeProcessor(llm=self.llm, settings=self.settings)
    ...
    def process_input(
        ...
        if input_type not in self.processors:
            logger.warning(
                f"Unsupported input type: {input_type}. Falling back to text."
            )
            input_type = "text"
    ...
    def detect_domain(self, processed_input: Dict[str, Any]) -> str:
        ...
        messages = [
            SystemMessage(
                content="You are an AI assistant that categorizes problems into domains."
            ),
        ...
        if detected_domain not in self.domain_solvers:
            logger.warning(
                f"Detected domain '{detected_domain}' not available. Falling back to general."
            )
    ...
    def solve_problem(
        ...
        solution["reasoning_steps"] = self.reasoning.get_reasoning_chain(
            processed_input, solution
        )
        ...
        logger.info(
            f"Problem solved. Domain: {domain}, Solution ID: {solution.get('id')}"
        )
        return solution
    ...
    def refine_solution(self, feedback: str) -> Dict[str, Any]:
        ...
        self.current_plan = self.planning.refine_plan(
            self.current_plan, processed_feedback
        )

        domain_solver = self.domain_solvers[domain]
        refined_solution = domain_solver.solve(
        ...
        refined_solution["reasoning_steps"] = self.reasoning.get_reasoning_chain(
            self.current_problem["input"], refined_solution, feedback=processed_feedback
        )
        ...
        logger.info(
            f"Solution refined. Domain: {domain}, Refined Solution ID: {refined_solution.get('id')}"
        )
    ...
    def explain_solution(self, solution_id: Optional[str] = None) -> Dict[str, Any]:
        if solution_id:
            solution = next(
                (s for s in self.solution_history if s.get("id") == solution_id), None
            )
            if not solution:
    ...
    def verify_solution(self, solution_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        logger.info(
            f"Verified solution: {solution.get('id')}, Result: {verification.get('is_correct')}"
        )
        return verification
    ...
    def get_alternative_solution(self) -> Dict[str, Any]:
        ...
        domain_solver = self.domain_solvers[domain]
        alternative_solution = domain_solver.solve(
            self.current_problem["input"], alternative_plan
        )

        alternative_solution["reasoning_steps"] = self.reasoning.get_reasoning_chain(
            self.current_problem["input"], alternative_solution, is_alternative=True
        )
        ...
        logger.info(
            f"Generated alternative solution. Domain: {domain}, Solution ID: {alternative_solution.get('id')}"
        )
