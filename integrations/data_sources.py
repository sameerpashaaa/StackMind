...
def _check_rate_limit(source: str) -> bool:
    ...
    REQUEST_TIMESTAMPS[source] = [
        ts for ts in REQUEST_TIMESTAMPS[source] if now - ts <= source_limits["period"]
    ]
...
def fetch_rest_api(
    ...
    try:
        ...
        response = requests.request(
            method=method, url=url, params=params, headers=headers, timeout=10
        )
...
def fetch_financial_data(
    symbol: str, period: str = "1y", interval: str = "1d", cache: bool = True
) -> Dict:
    """
    ...
    try:
        ...
        if query:
            data = newsapi.get_everything(
                q=query, sources=sources, language=language, page_size=page_size
            )
        else:
            data = newsapi.get_top_headlines(
                sources=sources, language=language, page_size=page_size
            )
