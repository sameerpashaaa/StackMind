...
from core.code_execution import (execute_javascript_code, execute_python_code,
...
def get_available_tools(settings) -> List[BaseTool]:
    ...
    if integrations_settings.get(
        "enable_code_execution", True
    ) and not integrations_settings.get("safe_mode", True):
        tools.append(
            LangChainTool(
                name="shell_executor",
...
def register_custom_tool(name: str, func: callable, description: str) -> BaseTool:
    ...
    """
    try:
        tool = LangChainTool(name=name, func=func, description=description)
