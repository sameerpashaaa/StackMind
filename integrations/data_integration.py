...
class DataIntegration:
    ...
    def __init__(self, cache_dir: Optional[str] = None, cache_ttl: int = 3600):
        ...
        self.cache_dir = cache_dir or os.path.join(
            os.path.expanduser("~"), ".ai_problem_solver", "cache"
        )
    ...
    def _load_api_keys(self):
        ...
        config_path = os.path.join(
            os.path.expanduser("~"), ".ai_problem_solver", "config", "api_keys.json"
        )
        if os.path.exists(config_path):
            try:
                ...
                logger.warning(
                    f"Could not load API keys from configuration file: {str(e)}"
                )
    ...
    def _check_rate_limit(self, api_name: str, limit_per_minute: int = 60) -> bool:
        ...
        self.api_rate_limits[api_name] = [
            t for t in self.api_rate_limits[api_name] if t > minute_ago
        ]
    ...
    def fetch_graphql(
        ...
        # Make the API request
        try:
            response = self.session.post(url=url, json=payload, headers=headers or {})
            response.raise_for_status()
    ...
    def scrape_webpage(
        ...
        try:
            ...
            if xpath:
                ...
                result["xpath_elements"] = [
                    etree.tostring(el).decode() if hasattr(el, "tag") else str(el)
                    for el in elements
                ]
    ...
    def fetch_stock_data(
        ...
        try:
            ...
            for symbol in symbols:
                ...
                data[symbol] = {"history": hist_dict, "info": info}
    ...
    def fetch_news(
        ...
        if "newsapi" in self.api_keys:
            return self._fetch_news_api(
                query, sources, category, limit, cache_key, use_cache
            )
        else:
    ...
    def _fetch_news_api(
        ...
        try:
            # Prepare parameters
            params = {"apiKey": self.api_keys["newsapi"], "pageSize": limit}
        ...
        try:
            ...
            for entry in all_entries:
                article = {
                    ...
                    "source": {"name": entry.get("source", {}).get("title", "Unknown")},
                }
    ...
    def fetch_weather(
        ...
        actual_cache_ttl = cache_ttl or min(
            self.cache_ttl, 3600
        )  # Max 1 hour for weather
        if use_cache:
            cached_data = self._get_from_cache(cache_key)
            if cached_data is not None:
        ...
        if "openweathermap" in self.api_keys:
            try:
                ...
                logger.error(
                    f"Error fetching weather data from OpenWeatherMap: {str(e)}"
                )
        ...
        try:
            ...
            url = (
                f"https://www.google.com/search?q=weather+{location.replace(' ', '+')}"
            )
            result = self.scrape_webpage(
                url, use_cache=False
            )  # Don't cache the intermediate result
    ...
    def query_database(
        ...
        if db_type.lower() != "sqlite":
            raise ValueError(
                f"Database type {db_type} is not supported yet. Only SQLite is supported."
            )
    ...
    async def fetch_multiple_apis(
        ...
        async def fetch_url(session, url, method, headers, params, data):
            try:
                if method.upper() == "GET":
                    ...
                    return {
                        "error": f"Method {method} not supported for async requests"
                    }
            except Exception as e:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for i, url in enumerate(urls):
                tasks.append(
                    fetch_url(session, url, method, headers, params[i], data[i])
                )
    ...
    def analyze_data(
        ...
        if not isinstance(data, pd.DataFrame):
            if isinstance(data, list) and all(isinstance(item, dict) for item in data):
                ...
                raise ValueError(
                    "Data must be a list of dictionaries, a dictionary, or a DataFrame."
                )
        else:
            df = data
    ...
    def _analyze_time_series(
        ...
        if not date_column:
            ...
            if not date_column:
                return {
                    "error": "Date column must be specified for time series analysis."
                }
        ...
        if not value_column:
            ...
            if not value_column:
                return {
                    "error": "Value column must be specified for time series analysis."
                }
        ...
        try:
            ...
            if len(ts_df) >= 4:  # Need at least a few data points
                try:
                    ...
                    result["seasonality"] = {"autocorrelation_lag1": float(acf_values)}
                except Exception as e:
